    ...
    |app.comp.page-editor $ {}
      ...
      :defs $ {}
                ...
                div ({})
                  ...
                  if
                    ...
                    span $ {} (:style style-link) (:inner-text "\"Reset")
                      :on-click $ fn (e d!)
                          :show confirm-reset-plugin
                          , d! $ fn () (on-reset-expr bookmark d!)
                  =< 8 nil
                  ...
                  span $ {} (:inner-text |Rename) (:style style-link)
                    :on-click $ fn (e d!)
                        :show rename-plugin
                        , d! $ fn (result) (on-rename-def result bookmark d!)
                  =< 8 nil
        ...
        |ui-missing $ quote
          def ui-missing $ div
            {} $ :style style-missing
            <> span "|Expression is missing!" nil
        |style-stack $ quote
      ...
      :defs $ {}
        ...
        |simulate-login! $ quote
          defn simulate-login! () $ let
            ...
            if (some? raw)
              do $ dispatch! :user/log-in (parse-cirru raw)
            ...
            let
              ...
              list-> :div
                {} (:tab-index 0)
                  ...
                  :style $ decide-expr-theme expr (contains? others coord) focused? tail? layout-mode (count coord) depth theme
                  :on $ if readonly?
                    {} $ :click
                      fn (e d!)
                        if picker-mode? $ do
                  ...
                  if (empty? children) result $ let-sugar
                    ...
                    recur
                      conj result $ [] k
                        if
                          = :leaf $ :type child
                          comp-leaf (>> states cursor-key) child focus child-coord (contains? partial-others child-coord) (= first-id k) readonly? picker-mode? theme
        ...
        |add-ns $ quote
          defn add-ns (db op-data session-id op-id op-time)
            let
              ...
              -> db
                assoc-in ([] :ir :files op-data) (assoc schema/file :ns default-expr :proc empty-expr)
                ...
                pre
                  {} $ :style
                    merge $ {} (:max-width "\"100%") (:overflow :auto)
                      :color $ hsl 0 0 60
                  code $ {}
                    :innerHTML $ trim (format-cirru configs)
                :ui version-plugin
                :ui modules-plugin
        |style-value $ quote
              ...
              if
                ...
                , db $ -> db
                  update-in
                    [] :sessions session-id :writer :stack (:pointer writer) :focus
                    fn (focus)
                      conj focus $ apply (if tail? max min)
                        keys $ :data target-expr
        |edit $ quote
                        ...
                        div
                          {} (:class-name |hoverable)
                            :style $ merge style-def
                              if
                                contains? highlights $ {} (:ns selected-ns) (:extra def-text) (:kind :def)
                                {} $ :color :white
                            :on-click $ fn (e d!)
                          ...
                          span
                            {} (:class-name "\"is-minor") (:style style-remove)
                              :on-click $ fn (e d!)
                                  :show confirm-remove-plugin
                                  , d! $ fn () (d! :ir/remove-def def-text)
            ...
            let
              ...
              if found?
                let
                    ...
                    data $ parse-cirru (fs/readFileSync storage-file "\"utf8")
